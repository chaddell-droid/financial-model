...
test('month 72 balance', () => eq(monthlyData[72].balance, -941024)); // B4 +$13.3k, A2 COLA +$28.6k, A1 haircut -$80.0k; 6.2 college tuition -$193.9k (34 mo x $5,666 from m=39); 6.4 medical trend -$38.4k ($4,200 premium at 6.5%/yr vs CPI 3%)
test('month 0 netCashFlow', () => eq(monthlyData[0].netCashFlow, -41455));
test('month 36 netCashFlow', () => eq(monthlyData[36].netCashFlow, -31088)); // B4 family rate; A2 COLA; A1 haircut -849 at m=36; 6.4 medical trend -484/mo at m=36
test('month 72 netCashFlow', () => eq(monthlyData[72].netCashFlow, -40934)); // A2 COLA +673; A1 haircut -914; 6.2 college -5,666/mo at m=72; 6.4 medical trend -1,113/mo at m=72
...
test('min balance is at month 12', () => {
  ...
  eq(minBal, -941024); // re-baselined for D7 gross-up, B4 (+$13.3k), A2 COLA (+$28.6k), A1 haircut (-$80.0k), 6.2 college (-$193.9k), 6.4 medical trend (-$38.4k)
...
// 6.4 medical trend -$37.8k (premium at 6.5%/yr vs CPI 3%).
test('month 72 balance', () => eq(deniedData[72].balance, -1393982));
...
const cappedBackPayScenario = gatherState({
  ...
  ssdiPersonal: 100,
  ssdiFamilyTotal: 100,  // disable auxiliary so this test isolates the attorney fee cap
});
...
test('month 12 balance', () => eq(debtData[12].balance, 19781)); // A2 COLA +769; A1 haircut (benefit tax m7-12 + back-pay tax 14,184) -19,653; 6.4 medical trend -983 over yr 1
// Re-baselined for D7 gross-up (was 0): the grossed-up draws now exhaust the
// reserves before month 72 even with debt retired.
test('month 72 balance', () => eq(debtData[72].balance, -406619)); // B4 +$13.3k, A2 COLA +$30.3k, A1 haircut -$87.0k; 6.2 college -$198.7k; 6.4 medical trend -$39.7k
...
test('month 72 balance', () => eq(cutsData[72].balance, -941024)); // B4 +$13.3k, A2 COLA +$28.6k, A1 haircut -$80.0k; 6.2 college -$193.9k; 6.4 medical trend -$38.4k
...
test('savings positive at Y6 - value', () => eq(goalResults[0].currentValue, -941024)); // B4 +$13.3k, A2 COLA +$28.6k, A1 haircut -$80.0k; 6.2 college -$193.9k; 6.4 medical trend -$38.4k
test('cash flow breakeven - fails', () => eq(goalResults[1].achieved, false));
test('cash flow breakeven - value', () => eq(goalResults[1].currentValue, -31088)); // B4; A2 COLA; A1 haircut at m=36; 6.4 medical trend -484/mo
...
// Re-baselined for D7 gross-up (was -487171).
test('savings positive at Y6 - value with cuts', () => eq(cutsGoalResults[0].currentValue, -941024)); // B4 +$13.3k, A2 COLA +$28.6k, A1 haircut -$80.0k; 6.2 college -$193.9k; 6.4 medical trend -$38.4k
...
test('buildPrimaryLeversModel computes numeric summary totals at baseline', () => {
  ...
  eq(model.summary.monthlyOutflow, 54382);
...
test('buildPrimaryLeversModel keeps BCS delta outputs numeric at baseline and full-support edges', () => {
  ...
  eq(fullSupport.bcs.monthlyDeltaFromStatusQuo, 1533);
  eq(fullSupport.bcs.totalDeltaOverRemainingYears, 64400);
});
...
test('AppShell narrows the side rail width contract', () => {
  ...
  assert.ok(source.includes("railWidth") && source.includes("${railWidth}px"), 'AppShell should use dynamic rail width prop');
...
test('R5: denied SSDI balance at month 71', () => {
  ...
  const { monthlyData } = runMonthlySimulation(s);
  assert.strictEqual(monthlyData[71].balance, -1349075); // was -960352 pre-D7; 6.2 college -$187.0k; 6.4 medical trend -$36.6k
});
...
test('R6: van sold at month 6, balance at month 71', () => {
  ...
  const { monthlyData } = runMonthlySimulation(s);
  assert.strictEqual(monthlyData[71].balance, -782756); // was -477252 pre-A1 (taxability haircut -$79.5k), -505255 pre-A2, -518665 pre-B4, -333183 pre-D7; 6.2 college -$188.7k; 6.4 medical trend -$37.3k
});
...
test('R7: lifestyle cuts $5000 override, balance at month 71', () => {
  ...
  const { monthlyData } = runMonthlySimulation(s);
  assert.strictEqual(monthlyData[71].balance, -492854); // was -179291 pre-A1 (haircut -$84.0k), -208383 pre-A2, -222214 pre-B4, -19887 pre-D7; 6.2 college -$191.4k; 6.4 medical trend -$38.2k
});
...
test('R8: debt retired, balance at month 71', () => {
  ...
  const { monthlyData } = runMonthlySimulation(s);
  assert.strictEqual(monthlyData[71].balance, -372119); // was -54448 pre-A1 (haircut -$86.1k), -84087 pre-A2, -98156 pre-B4; 6.2 college -$193.0k; 6.4 medical trend -$38.6k
});
...
// R10 — SSDI back pay amount (includes auxiliary share for kids)
...
test('R12: one-time extras end at month 12 — month 12 expenses', () => {
  const s = gatherState({ oneTimeExtras: 5000, oneTimeMonths: 12 });
  const { monthlyData } = runMonthlySimulation(s);
  assert.strictEqual(monthlyData[12].expenses, 55844); // 6.4 (2026-06-10): premium trends 6.5%/yr vs CPI 3% → +147 at m=12
});
