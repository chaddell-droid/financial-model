...
test('59. Spend schedule single entry matches old totalMonthlySpend behavior', () => {
  const spend = 55000;
  const s = gatherStateWithOverrides({ spendSchedule: [{ month: 0, amount: spend }], lifestyleCutsApplied: false, vanSold: false });
  const { monthlyData } = runMonthlySimulation(s);
  near(monthlyData[0].expenses, spend, 1, 'month 0 expenses should reconstruct to spend schedule amount');
});

test('60. Spend schedule step-down at month 12', () => {
  const s = gatherStateWithOverrides({
    spendSchedule: [{ month: 0, amount: 60000 }, { month: 12, amount: 50000 }],
    lifestyleCutsApplied: false, vanSold: false,
  });
  const { monthlyData } = runMonthlySimulation(s);
  near(monthlyData[0].expenses, 60000, 1, 'month 0 at first level');
  near(monthlyData[11].expenses, 60000, 1, 'month 11 still first level');
  near(monthlyData[12].expenses, 50000, 1, 'month 12 steps down to second level');
  near(monthlyData[13].expenses, 50000, 1, 'month 13 stays at second level');
});

test('61. Spend schedule with retireDebt reduces expenses', () => {
  const spend = 55000;
  const s = gatherStateWithOverrides({ spendSchedule: [{ month: 0, amount: spend }], retireDebt: true, lifestyleCutsApplied: false, vanSold: false });
  const { monthlyData } = runMonthlySimulation(s);
  near(monthlyData[0].expenses, spend - s.debtService, 1, 'expenses should be spend minus debt service');
});

test('62. Empty spend schedule falls back to baseExpenses', () => {
  const s = gatherStateWithOverrides({ spendSchedule: [], lifestyleCutsApplied: false, vanSold: false });
  const { monthlyData } = runMonthlySimulation(s);
  // Should use baseExpenses + debtService + vanMonthlySavings + bcsFamilyMonthly (month 0 with BCS active)
  const expected = s.baseExpenses + s.debtService + s.vanMonthlySavings + s.bcsFamilyMonthly;
  near(monthlyData[0].expenses, expected, 1, 'should use component-based expenses');
});
...
test('63. One-time expense deducts from balance at correct month', () => {
  const s = gatherStateWithOverrides({ oneTimeExpenses: [{ name: 'Test', month: 6, amount: 10000 }] });
  const { monthlyData } = runMonthlySimulation(s);
  const balBefore = monthlyData[5].balance;
  const cashFlow6 = monthlyData[6].cashIncome + monthlyData[6].investReturn - monthlyData[6].expenses;
  const expected = balBefore + cashFlow6 - 10000;
  near(monthlyData[6].balance, expected, 1, 'balance at month 6 should reflect one-time deduction');
});

test('64. Multiple one-time expenses at different months', () => {
  const s = gatherStateWithOverrides({
    oneTimeExpenses: [
      { name: 'A', month: 3, amount: 5000 },
      { name: 'B', month: 6, amount: 8000 },
    ],
  });
  const { monthlyData } = runMonthlySimulation(s);
  // Check that month 3 has a 5000 hit (compare to a baseline without one-time)
  const baseline = runMonthlySimulation(gatherStateWithOverrides({ oneTimeExpenses: [] }));
  const diff3 = baseline.monthlyData[3].balance - monthlyData[3].balance;
  near(diff3, 5000, 1, 'month 3 should be 5000 lower than baseline');
});

test('65. One-time expense at month 0', () => {
  const s = gatherStateWithOverrides({ oneTimeExpenses: [{ name: 'Immediate', month: 0, amount: 15000 }] });
  const baseline = runMonthlySimulation(gatherStateWithOverrides({ oneTimeExpenses: [] }));
  const { monthlyData } = runMonthlySimulation(s);
  const diff = baseline.monthlyData[0].balance - monthlyData[0].balance;
  near(diff, 15000, 1, 'month 0 balance should be 15000 lower');
});

test('66. One-time expenses do NOT affect monthly expense line', () => {
  const s = gatherStateWithOverrides({ oneTimeExpenses: [{ name: 'Test', month: 3, amount: 50000 }] });
  const baseline = runMonthlySimulation(gatherStateWithOverrides({ oneTimeExpenses: [] }));
  const { monthlyData } = runMonthlySimulation(s);
  assert.strictEqual(monthlyData[3].expenses, baseline.monthlyData[3].expenses,
    'expenses field should be identical with and without one-time events');
});
